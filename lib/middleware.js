 ...
 * an expired or forged token is rejected without a network call. It is skipped
 * for a token whose algorithm this service holds no key for, because there it is
 * not evidence of anything — see canPreCheck.
 *
 * Options:
...
function createAuthMiddleware(options = {}) {
  ...
  const mw = async function authenticate(req, res, next) {
    ...
    // Skipped when this service holds no key for the token's algorithm — see
    // canPreCheck. There is a remote verifier behind this one, and an
    // optimisation must not be what refuses a session the authority would
    // accept. Everything else is still rejected here, for free.
...
// Local mode has NOTHING behind it, so unlike the remote path above it must keep
// rejecting a token it cannot verify. "I hold no key for this algorithm" is a
// misconfiguration, and the only alternative to refusing is admitting an
// unverified session — which is worse than an outage.
